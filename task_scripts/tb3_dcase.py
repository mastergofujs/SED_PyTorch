...
def running(options):
    """
    This function is defined to implement the training, testing and evaluation stages.
    :param options: the optional parameters
    :return: none, but will save the model and SED results.
    """
    ...
    for k in range(1, folds + 1):
        ...
        for e in range(epoch):
            ...
            for n_sample, (x_data, y_data) in enumerate(train_loader):
                ...
                l_disent, (l_detector, _) = disent_loss((detectors_out, z_stars, alphas, (mu, log_var)), y_data.float().cuda())
                ...
                optimizer.step()
                if n_sample % 500 == 0:
                    print('--- epoch/epochs: %d/%d || batch/batches: %d/%d ---' % (e, epoch, n_sample, len(train_loader)))
            ...
            if epoch == 0:
                ...
                if val_loss < min_loss:
                    ...
                    min_loss = val_loss
                    best_e = e
                    torch.save(sb_vae.state_dict(), options.result_path + sb_vae.name + '/fold_' + str(k) + '_cp_weight.h5')
                else:
                    patience += 1
                    print('Validation loss dose not decreased than {}.'.format(min_loss))

        if not epoch == 0:
            print('Fold ' + str(k) + 'trained done, best validation results is: f1={}, er={}, loss={}, e={}'.format(
                best_f1, best_er, min_loss, best_e
            ))
